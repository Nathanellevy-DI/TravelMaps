...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderMemories(memories) {
        if (!memories || memories.length === 0) {
            return '<p class="result-sub">No memories yet.</p>';
        }
        ...
        document.querySelectorAll('.delete-memory').forEach(btn => {
            btn.onclick = () => {
                ...
                if (confirm('Delete this memory?')) {
                    place.memories = place.memories.filter(m => m.id !== id);
                    savePlaces();
                    document.getElementById('memories-grid').innerHTML = renderMemories(place.memories);
                    setupMemoryListeners(place);
                    renderSavedList();
    ...
    function renderSavedList() {
        ...
        if (!savedPlaces || savedPlaces.length === 0) { 
            savedList.innerHTML = '<div class="result-sub">No saved places yet.</div>'; 
            return; 
        }
        ...
        savedPlaces.forEach(place => {
            ...
            card.innerHTML = `
                <div class="saved-left">
                    <div class="saved-title">${place.name}</div>
                    <div class="saved-sub">${memCount} memories</div>
                </div>
                <div class="saved-actions">
                    <button class="small-btn mem-trigger">📸</button>
                    <button class="small-btn go-trigger">Go</button>
                    <button class="small-btn del-trigger">Delete</button>
                </div>`;
            
            card.querySelector('.mem-trigger').onclick = () => openMemoriesModal(place.id);
            card.querySelector('.go-trigger').onclick = () => {
                map.setView([place.lat, place.lon], 15);
                if (savedMarkers[place.id]) savedMarkers[place.id].openPopup();
            };
            card.querySelector('.del-trigger').onclick = () => {
    ...
    function addSavedMarker(place) {
        ...
        m.bindPopup(`<strong>${place.name}</strong><br><button class="primary" onclick="window.openMemoriesFromPopup('${place.id}')">View Memories</button>`);
        savedMarkers[place.id] = m;
    }

    window.openMemoriesFromPopup = (id) => openMemoriesModal(id);
